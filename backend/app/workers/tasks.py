...
def ingest_call(self, project_id: str, provider: str, provider_call_id: str):
    """
    Ingests call details from a speech provider.
    1. Checks if call has already been ingested (Idempotency check).
    2. Fetches integration credentials (decrypted).
    3. Calls provider API to get duration, audio_url, agent name, and speaker turns.
    4. Resolves agent (creates if not found).
    5. Creates conversation in 'Processing' state.
    6. Saves speaker turns as speech segments.
    7. Chains evaluate_audio task.
    """
    ...
    try:
        ...
        # Idempotency check: return existing conversation if already ingested
        ...
        if existing:
            logger.info(f"Call '{provider_call_id}' already ingested as conversation {existing.id}. Skipping duplicate insertion.")
...
def sync_calls_task(self, project_id: str, provider: str):
    """
...
def evaluate_audio(self, conversation_id: str, audio_url: str):
    """
    ...
    try:
        ...
        logger.warning(f"Failed to run real evaluation pipeline ({pipeline_err}). Falling back to dynamic segment evaluation.")
